...
// ============================================================
// CONFIGURATION: Set to true to show free HexColorPicker,
// set to false to show predefined color swatches only.
// ============================================================
const ENABLE_FREE_COLOR_PICKER = false;

// Predefined color palettes for each product
const productColors = {
  Shoe: {
...
// Fallback palette if product/part not defined above
const defaultColors = ["#ffffff", "#000000", "#d3d3d3", "#a8a8a8", "#ff0000", "#0055ff", "#ffcc00", "#00cc44"];

export default function ColorPicker({ state, updateColor, modelName }) {
  const snap = useSnapshot(state);

  const getSwatches = () => {
    if (modelName && snap.current && productColors[modelName]?.[snap.current]) {
      return productColors[modelName][snap.current];
    }
  ...
  return (
    <div className={snap.current !== null ? "color-picker" : "color-picker hidden"}>
      ...
      {/* Free Color Picker - enable by setting ENABLE_FREE_COLOR_PICKER = true */}
